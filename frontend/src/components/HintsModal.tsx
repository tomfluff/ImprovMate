...
const HintsModal = ({
    ...
    queryFn: ({ signal }) => {
      ...
      if (ending) {
        return instance
          .post(
            "/story/end_hints",
            { language: "en", context: createCallContext({}) },
            { signal }
          ) //TODO: Add in createCallContext
          .then((res) => {
  ...
  return (
    //TODO: render next to the camera window
    <Modal
      size="lg"
      opened={display}
      onClose={finalAction}
      ...
      <Container>
        ...
        {hintList && hintList.length > 0 && (
          <Accordion chevron={<FaPlus />}>
            {!ending &&
              hintList.length > 0 &&
              Object.keys(hintList[0]).map((category) => (
                <Accordion.Item key={category} value={category}>
                  ...
                  <Accordion.Panel>
                    {hintList.map(
                      (hint: { [key: string]: string }, index: number) => (
                        <Box key={index} mb="sm">
                          <Grid align="center">
                            <Grid.Col span={10}>
                              <span>
                            ...
                            <Grid.Col span={2}>
                              <Button
                                size="xs"
                                onClick={() =>
                                  handleSelectHint(category, hint[category])
                                }
                                disabled={
                                  selectedHints[category] !== undefined &&
                                  hint[category] !== undefined &&
                                  selectedHints[category] === hint[category]
                                }
                              >
                                Select
                              </Button>
                          ...
                          <Grid align="center">
                            ...
                            <Grid.Col span={2}>
                              <Button
                                size="xs"
                                onClick={() =>
                                  handleSelectEndHint(category, hint[category])
                                }
                                disabled={Object.values(selectedHints).includes(
                                  hint[category]
                                )}
                              >
                                Select
